...
def points_transform_(points, centers, point_masks, loc_transform,
    ...
    for i in range(num_box):
        rot_mat_T[i] = _rotation_matrix_3d_(rot_mat_T[i], rot_transform[i], 2)
...
def set_group_noise_same_v2_(loc_noise, rot_noise, grot_noise, group_ids):
    ...
    return loc_noise,rot_noise,grot_noise
...
def noise_per_object_v3_(gt_boxes,
    ...
    if group_ids is not None:
        if enable_grot:
            loc_noise,rot_noise,grot_noise = set_group_noise_same_v2_(loc_noises, rot_noises, global_rot_noises,
                                     group_ids)
        else:
            loc_noise,rot_noise = set_group_noise_same_(loc_noises, rot_noises, group_ids)
        group_centers, group_id_num_dict = get_group_center(
            gt_boxes[:, :3], group_ids)
        if enable_grot:
            loc_noises = group_transform_v2_(loc_noises, rot_noises, gt_boxes[:, :3],
                                gt_boxes[:, 6], group_centers,
                                global_rot_noises, valid_mask)
        else:
            loc_noises = group_transform_(loc_noises, rot_noises, gt_boxes[:, :3],
                             gt_boxes[:, 6], group_centers, valid_mask)
        group_nums = list(group_id_num_dict.values())
        group_nums = np.array(group_nums, dtype=np.int64)
    ...
    if points is not None:
        ...
        points = points_transform_(points, gt_boxes[:, :3], point_masks, loc_transforms,
                          rot_transforms, valid_mask)

    gt_boxes = box3d_transform_(gt_boxes, loc_transforms, rot_transforms, valid_mask)

    return gt_boxes,points,valid_mask
