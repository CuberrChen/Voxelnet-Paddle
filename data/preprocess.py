...
# @profile(precision=4,stream=open('merge_second_batch_memory_profiler.log','w+')) #only for debug
def merge_second_batch(batch_list, _unused=False):
    ...
    for key, elems in example_merged.items():
        if key in [
            ...
            ret[key] = np.concatenate(elems, axis=0) # TODO fix memory leak bug
            ...
            ret[key] = np.stack(elems, axis=0) # TODO fix memory leak bug
    return ret

# @profile(precision=4,stream=open('prep_pointcloud_memory_profiler.log','w+')) #only for debug
def prep_pointcloud(input_dict,
    ...
    if training:
        ...
        gt_boxes,points,gt_boxes_mask= prep.noise_per_object_v3_( # TODO FIX 
            gt_boxes,
            points,
            gt_boxes_mask,
    ...
    voxels, coordinates, num_points = voxel_generator.generate(
        points, max_voxels) # TODO fix memory leak bug
...
# @profile(precision=4,stream=open('_read_and_prep_v9_memory_profiler.log','w+')) #only for debug
def _read_and_prep_v9(info, root_path, num_point_features, prep_func):
    ...
    example = prep_func(input_dict=input_dict) # TODO fix memory leak bug
...
def example_to_tensorlist(example, cfg, float_type=paddle.float32):
    ...
    if 'anchors_mask' in example.keys():
        example_list[12]=paddle.to_tensor(example['anchors_mask'], dtype=paddle.bool)
...
def example_to_tensorlist_without_batch(example,cfg,float_type=paddle.float32):
    example_list = [None] * 13
    pillar_x = example['voxels'][:, :, 0][np.newaxis,np.newaxis,:,:] # (N,1,K,T)
...
def example_convert_to_paddle(example, dtype=paddle.float32) -> dict:
    ...
    for k, v in example.items():
        if k in float_names:
            # slow when directly provide fp32 data with dtype=paddle.half
            example_paddle[k] = paddle.to_tensor(
                v, dtype=paddle.float32).to(dtype)
            ...
            for k1, v1 in v.items():
                calib[k1] = paddle.to_tensor(
                    v1, dtype=dtype).to(dtype)
            example_paddle[k] = calib
        elif k == "num_voxels":
            example_paddle[k] = paddle.to_tensor(v)
        else:
            example_paddle[k] = v
