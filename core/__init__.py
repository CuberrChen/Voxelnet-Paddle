...
def build_optimizer(optimizer_config, params, name=None):
  ...
  """
    ...
    if optimizer_config.name =='sgd_optimizer':
...
def _get_base_lr_by_lr_scheduler(learning_rate_config):
    ...
    if learning_rate_type == 'exponential_decay_learning_rate':
        ...
        base_lr = paddle.optimizer.lr.ExponentialDecay(learning_rate=learning_rate_config.initial_learning_rate,gamma=learning_rate_config.gamma)

    if learning_rate_type == 'manual_step_learning_rate':
        
        base_lr = paddle.optimizer.lr.MultiStepDecay(learning_rate=learning_rate_config.initial_learning_rate, milestones=learning_rate_config.milestones, gamma=learning_rate_config.gamma)

    if learning_rate_type == 'cosine_decay_learning_rate':

        base_lr = paddle.optimizer.lr.CosineAnnealingDecay(learning_rate=learning_rate_config.initial_learning_rate, T_max=learning_rate_config.T_max, eta_min=learning_rate_config.eta_min)
    
    if learning_rate_type == 'polynomialdecay':

        base_lr = paddle.optimizer.lr.PolynomialDecay(learning_rate = learning_rate_config.initial_learning_rate, decay_steps = learning_rate_config.decay_steps,
        end_lr = learning_rate_config.end_lr, power = learning_rate_config.power)
